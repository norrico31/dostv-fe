...
import { deviceDao } from '../../shared/dao/system-settings/DeviceDao';
...
const { getDevices, postDevice, putDevice, deleteDevice } = deviceDao()
...
function Modal({ open, onClose, selectedData, getData }: Props) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = id ? putDevice({ id, ...fields }) : postDevice(fields)
            await res
            setInfo({
                status: id ? 'warning' : 'success',
                message: `${id ? 'Update' : 'Create'} Device Successfully`
            })
                ...
                >
                    <div className="mt-3 p-3 sm:mt-0 text-left">
                        <DialogTitle as="h2" className="text-2xl font-bold leading-6 text-gray-700 dark:text-white">
                            Device - {selectedData ? 'Edit' : 'Create'}
                        </DialogTitle>
                        <hr className='my-5' />
...
export default function Devices() {
    ...
    const [{ data, ...pageProps }, setData] = useState<DaoFE<Device[]>>(initState)
    const [search, inputValue, onChange] = useSearchDebounce()
    const [openModal, setOpenModal] = useState(false)
    const [selectedData, setSelectedData] = useState<Device | undefined>(undefined)
    ...
    async function getData(args?: ApiParams) {
        setLoading(true)
        try {
            const res = await getDevices({ signal: args?.signal, search: args?.search, page: args?.page });
            setData(res)
        } catch (error) {
    ...
    function onEditHandler(selectedData: Device) {
        setSelectedData(selectedData)
        setOpenModal(true)
    }

    function onDeleteHandler(selectedData: Device) {
        setSelectedData(selectedData)
        setOpenDeleteModal(true)
    }

    async function deleteData(id: string) {
        await deleteDevice(id).finally(getData).finally(closeModal)
    }
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-2 flex-wrap">
                <h2 className="heading-2 mb-1">Device / Browser</h2>
