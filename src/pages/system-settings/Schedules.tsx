...
import { scheduleDao } from '../../shared/dao/system-settings/ScheduleDao';
...
const { getSchedules, postSchedule, putSchedule, deleteSchedule } = scheduleDao()
...
function Modal({ open, onClose, selectedData, getData }: Props) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = id ? putSchedule({ id, ...fields }) : postSchedule(fields)
            await res
            setInfo({
                status: id ? 'warning' : 'success',
                message: `${id ? 'Update' : 'Create'} Schedule Successfully`
            })
                ...
                >
                    <div className="mt-3 p-3 sm:mt-0 text-left">
                        <DialogTitle as="h2" className="text-2xl font-bold leading-6 text-gray-700 dark:text-white">
                            Schedule - {selectedData ? 'Edit' : 'Create'}
                        </DialogTitle>
                        <hr className='my-5' />
...
const initState: DaoFE<Schedule[]> = {
    data: [],
    page: 1,
    lastPage: 1,
    totalItems: 0
...
export default function Schedules() {
    ...
    const [{ data, ...pageProps }, setData] = useState<DaoFE<Schedule[]>>(initState)
    const [search, inputValue, onChange] = useSearchDebounce()
    const [openModal, setOpenModal] = useState(false)
    const [selectedData, setSelectedData] = useState<Schedule | undefined>(undefined)
    ...
    async function getData(args?: ApiParams) {
        setLoading(true)
        try {
            const res = await getSchedules({ signal: args?.signal, search: args?.search, page: args?.page });
            setData(res)
        } catch (error) {
    ...
    function onEditHandler(selectedData: Schedule) {
        setSelectedData(selectedData)
        setOpenModal(true)
    }

    function onDeleteHandler(selectedData: Schedule) {
        setSelectedData(selectedData)
        setOpenDeleteModal(true)
    }

    async function deleteData(id: string) {
        await deleteSchedule(id).finally(getData).finally(closeModal)
    }
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-2 flex-wrap">
                <h2 className="heading-2 mb-1">Schedules</h2>
            ...
            <div>
                <Table>
                    ...
                    <tbody>
                        {loading ? <tr><td className='w-full h-full'><Loading /></td></tr> : (data ?? []).map((d) => (
                            <tr className="bg-white dark:bg-gray-900/50 hover:bg-gray-100/50 dark:hover:bg-gray-900" key={d.id}>
