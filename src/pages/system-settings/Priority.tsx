...
import { priorityDao } from '../../shared/dao/system-settings/PriorityDao';
...
const { getPriorities, postPriority, putPriority, deletePriority } = priorityDao()
...
function Modal({ open, onClose, selectedData, getData }: Props) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = id ? putPriority({ id, ...fields }) : postPriority(fields)
            await res
            setInfo({
                status: id ? 'warning' : 'success',
                message: `${id ? 'Update' : 'Create'} Priority Successfully`
            })
                ...
                >
                    <div className="mt-3 p-3 sm:mt-0 text-left">
                        <DialogTitle as="h2" className="text-2xl font-bold leading-6 text-gray-700 dark:text-white">
                            Priority - {selectedData ? 'Edit' : 'Create'}
                        </DialogTitle>
                        <hr className='my-5' />
...
const initState: DaoFE<Priority[]> = {
    data: [],
    page: 1,
    lastPage: 1,
    totalItems: 0
...
export default function Priority() {
    ...
    const [data, setData] = useState<DaoFE<Priority[]>>(initState)
    const [search, inputValue, onChange] = useSearchDebounce()
    const [openModal, setOpenModal] = useState(false)
    const [selectedData, setSelectedData] = useState<Priority | undefined>(undefined)
    ...
    async function getData(args?: ApiParams) {
        setLoading(true)
        try {
            const res = await getPriorities({ signal: args?.signal, search: args?.search, page: args?.page });
    ...
    function onEditHandler(selectedData: Priority) {
        setSelectedData(selectedData)
        setOpenModal(true)
    }

    function onDeleteHandler(selectedData: Priority) {
        setSelectedData(selectedData)
        setOpenDeleteModal(true)
    }

    async function deleteData(id: string) {
        await deletePriority(id).finally(getData).finally(closeModal)
    }
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-2 flex-wrap">
                <h2 className="heading-2 mb-1">Prioritys</h2>
