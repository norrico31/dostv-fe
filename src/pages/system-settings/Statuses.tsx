...
import { statusDao } from '../../shared/dao/system-settings/StatusDao';
...
const { getStatuses, postStatus, putStatus, deleteStatus } = statusDao()
...
function Modal({ open, onClose, selectedData, getData }: Props) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = id ? putStatus({ id, ...fields }) : postStatus(fields)
            await res
            setInfo({
                status: id ? 'warning' : 'success',
                message: `${id ? 'Update' : 'Create'} Status Successfully`
            })
    ...
    return <Dialog className="relative z-10" open={open} onClose={onClose}>
        ...
        <div className="fixed inset-0 z-10 w-screen">
            <div className="flex min-h-full items-center justify-center text-center sm:items-center px-2">
                <DialogPanel
                    transition
                    className="relative p-5 transformSche rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all data-[closed]:translate-y-4 data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in sm:my-8 w-full sm:w-full sm:max-w-lg data-[closed]:sm:translate-y-0 data-[closed]:sm:scale-95"
                >
                    <div className="mt-3 p-3 sm:mt-0 text-left">
                        <DialogTitle as="h2" className="text-2xl font-bold leading-6 text-gray-700 dark:text-white">
                            Status - {selectedData ? 'Edit' : 'Create'}
                        </DialogTitle>
                        <hr className='my-5' />
...
export default function Statuses() {
    ...
    function onEditHandler(selectedData: Status) {
        setSelectedData(selectedData)
        setOpenModal(true)
    }

    function onDeleteHandler(selectedData: Status) {
        setSelectedData(selectedData)
        setOpenDeleteModal(true)
    }

    async function deleteData(id: string) {
        await deleteStatus(id).finally(getData).finally(closeModal)
    }
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-2 flex-wrap">
                <h2 className="heading-2 mb-1">Status</h2>
