...
import { issueTypesDao } from '../../shared/dao/system-settings/IssueTypeDao';
...
const { getIssueTypes, postIssueType, putIssueType, deleteIssueType } = issueTypesDao()
...
function Modal({ open, onClose, selectedData, getData }: Props) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = id ? putIssueType({ id, ...fields }) : postIssueType(fields)
            await res
            setInfo({
                status: id ? 'warning' : 'success',
                message: `${id ? 'Update' : 'Create'} Issue Type Successfully`
            })
                ...
                >
                    <div className="mt-3 p-3 sm:mt-0 text-left">
                        <DialogTitle as="h2" className="text-2xl font-bold leading-6 text-gray-700 dark:text-white">
                            Issue type - {selectedData ? 'Edit' : 'Create'}
                        </DialogTitle>
                        <hr className='my-5' />
...
const initState: DaoFE<IssueType[]> = {
    data: [],
    page: 1,
    lastPage: 1,
    totalItems: 0
...
export default function IssueTypes() {
    ...
    const [{ data, ...pageProps }, setData] = useState<DaoFE<IssueType[]>>(initState)
    const [search, inputValue, onChange] = useSearchDebounce()
    const [openModal, setOpenModal] = useState(false)
    const [selectedData, setSelectedData] = useState<IssueType | undefined>(undefined)
    ...
    async function getData(args?: ApiParams) {
        setLoading(true)
        try {
            const res = await getIssueTypes({ signal: args?.signal, search: args?.search, page: args?.page });
            setData(res)
        } catch (error) {
    ...
    function onEditHandler(selectedData: IssueType) {
        setSelectedData(selectedData)
        setOpenModal(true)
    }

    function onDeleteHandler(selectedData: IssueType) {
        setSelectedData(selectedData)
        setOpenDeleteModal(true)
    }

    async function deleteData(id: string) {
        await deleteIssueType(id).finally(getData).finally(closeModal)
    }
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-2 flex-wrap">
                <h2 className="heading-2 mb-1">Issue Types</h2>
