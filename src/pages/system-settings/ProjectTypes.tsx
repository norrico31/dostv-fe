...
import { projectTypesDao } from '../../shared/dao/system-settings/ProjectTypeDao';
...
const { getProjectTypes, postProjectType, putProjectType, deleteProjectType } = projectTypesDao()
...
function Modal({ open, onClose, selectedData, getData }: Props) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = id ? putProjectType({ id, ...fields }) : postProjectType(fields)
            await res
            setInfo({
                status: id ? 'warning' : 'success',
                message: `${id ? 'Update' : 'Create'} ProjectType Successfully`
            })
                ...
                >
                    <div className="mt-3 p-3 sm:mt-0 text-left">
                        <DialogTitle as="h2" className="text-2xl font-bold leading-6 text-gray-700 dark:text-white">
                            Project type - {selectedData ? 'Edit' : 'Create'}
                        </DialogTitle>
                        <hr className='my-5' />
...
const initState: DaoFE<ProjectType[]> = {
    data: [],
    page: 1,
    lastPage: 1,
    totalItems: 0
...
export default function ProjectTypes() {
    ...
    const [{ data, ...pageProps }, setData] = useState<DaoFE<ProjectType[]>>(initState)
    const [search, inputValue, onChange] = useSearchDebounce()
    const [openModal, setOpenModal] = useState(false)
    const [selectedData, setSelectedData] = useState<ProjectType | undefined>(undefined)
    ...
    async function getData(args?: ApiParams) {
        setLoading(true)
        try {
            const res = await getProjectTypes({ signal: args?.signal, search: args?.search, page: args?.page });
            setData(res)
        } catch (error) {
    ...
    function onEditHandler(selectedData: ProjectType) {
        setSelectedData(selectedData)
        setOpenModal(true)
    }

    function onDeleteHandler(selectedData: ProjectType) {
        setSelectedData(selectedData)
        setOpenDeleteModal(true)
    }

    async function deleteData(id: string) {
        await deleteProjectType(id).finally(getData).finally(closeModal)
    }
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-2 flex-wrap">
                <h2 className="heading-2 mb-1">Project Types</h2>
